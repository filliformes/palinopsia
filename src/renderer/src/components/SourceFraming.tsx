...
export function SourceFraming({
  ...
  return (
    <div className="flex flex-col gap-1 border-t border-border px-2 py-2">
      ...
      <Slider label="pan x" value={panX} min={-1} max={1} def={0} onChange={(v) => set({ panX: v })} />
      <Slider label="pan y" value={panY} min={-1} max={1} def={0} onChange={(v) => set({ panY: v })} />
      <Slider label="crop l" value={cropL} min={0} max={0.9} def={0} onChange={(v) => set({ cropL: v })} />
      <Slider label="crop r" value={cropR} min={0} max={0.9} def={0} onChange={(v) => set({ cropR: v })} />
      <Slider label="crop t" value={cropT} min={0} max={0.9} def={0} onChange={(v) => set({ cropT: v })} />
      <Slider label="crop b" value={cropB} min={0} max={0.9} def={0} onChange={(v) => set({ cropB: v })} />
    </div>
  )
}
