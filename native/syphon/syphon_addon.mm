...
// send(buffer, width, height). RGBA8, GL bottom-up : Syphon flips it (flipped:YES).
static Napi::Value Send(const Napi::CallbackInfo& info) {
  ...
  @autoreleasepool {
    ...
    [g_server publishFrameTexture:g_tex onCommandBuffer:cb imageRegion:NSMakeRect(0, 0, w, h) flipped:YES];
    [cb commit];
    g_last = cb;
  }
